...
class WorkLogController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $maintenanceRequest = MaintenanceRequest::with(['items', 'assignedTechnicians'])
            ->findOrFail($validated['request_id']);


        // Check if technician is assigned to these items
        $assignment = MaintenanceRequestTechnician::where('maintenance_request_id', $maintenanceRequest->id)
        ...
        // Check if technician is assigned to the selected items
        $assignedItemIds = $assignment->item_ids ?? [];
        foreach ($validated['item_ids'] as $itemId) {
            if (!in_array($itemId, $assignedItemIds)) {
                return response()->json([
                    'success' => false,
                    'message' => 'You are not assigned to one or more selected items.'
                ], 403);
            }
        }
        try {
            ...
            // Get issue types for selected items
            $selectedIssueTypeIds = $maintenanceRequest->items
            ...
            // Create work log with item tracking
            $workLog = WorkLog::create([
                'request_id' => $validated['request_id'],
                'technician_id' => $user->id,
                'work_done' => $validated['work_done'],
                ...
                'item_notes' => $validated['item_notes'] ?? [],
            ]);

            // Update request status logic based on items
            $allItemsCount = $maintenanceRequest->items->count();
            $workedOnItemsCount = count($validated['item_ids']);
            ...
            if ($validated['new_status'] === 'completed') {
                ...
                if ($workedOnItemsCount === $allItemsCount) {
                    // All items worked on - mark as completed
                    $statusUpdateData = [
                        'status' => 'completed',
                        'completed_at' => now(),
                    ];
                    ...
                    // Partial completion - keep as in_progress
                    $statusUpdateData = [
                        'status' => MaintenanceRequest::STATUS_IN_PROGRESS,
                    ];
                }
            } elseif ($validated['new_status'] === 'not_fixed') {
                // Mark as not fixed for review
                $statusUpdateData = [
                    'status' => MaintenanceRequest::STATUS_NOT_FIXED,
                    'completed_at' => now(),
                ];
            ...
            if (empty($remainingItems) && $validated['new_status'] === 'completed') {
                $assignment->update([
                    'status' => 'completed',
                    'completed_at' => now(),
                ]);
            } elseif ($validated['new_status'] === 'in_progress') {
                $assignment->update([
                    'status' => 'in_progress',
                    'started_at' => now(),
                ]);
