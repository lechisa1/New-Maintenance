    ...
    <div class="space-y-6">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <form action="{{ route('permissions.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
                <div class="relative">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search resource..."
                ...
                @if (request()->anyFilled(['search', 'status']))
                    <a href="{{ route('permissions.index') }}"
                        class="text-sm text-red-600 hover:underline dark:text-red-400">Clear Filters</a>
                @endif
            </form>
        </div>
                                        ...
                                        <td class="px-4 py-4 text-center">
                                            @php
                                                $perm = $permissions->first(
                                                    fn($p) => str_ends_with($p->name, '.' . $action) ||
                                                        $p->name == $action,
                                                );
                                            @endphp

                                            @if ($perm)
                                                <form action="{{ route('permissions.toggle', $perm->id) }}" method="POST"
                                                    class="inline-block">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit"
                                                        class="relative inline-flex h-5 w-10 flex-shrink-0 cursor-pointer items-center rounded-full transition-colors duration-200 focus:outline-none {{ $perm->is_active ? 'bg-green-600' : 'bg-gray-300 dark:bg-gray-700' }}">
                                                        <span class="sr-only">Toggle {{ $perm->name }}</span>
                                                        <span
                                                            class="inline-block h-4 w-4 transform rounded-full bg-white transition duration-200 ease-in-out {{ $perm->is_active ? 'translate-x-5' : 'translate-x-1' }}"></span>
                                                    </button>
                                                </form>
                                            @else
