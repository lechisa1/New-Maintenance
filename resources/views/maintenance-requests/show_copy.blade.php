                                        ...
                                        <div>
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                                ...
                                                <label
                                                    ...
                                                    <input type="radio" name="new_status" value="in_progress" required
                                                ...
                                                <label
                                                    ...
                                                    <input type="radio" name="new_status" value="completed" required
                                                ...
                                                <label
                                                    ...
                                                    <input type="radio" name="new_status" value="not_fixed" required
                                            ...
                                            <div>
                                                ...
                                                <div class="relative">
                                                    <input type="number" name="time_spent_minutes"
                                                        id="time_spent_minutes" required min="1" max="480"
                                                        step="15"
                                            ...
                                            <div class="flex items-start">
                                                ...
                                                <div class="text-sm">
                                                    ...
                                                    <ul
                                                        ...
                                                        <li><strong>In Progress:</strong> Still working on the issue</li>
                                        ...
                                        <div class="flex justify-between items-start mb-2">
                                            <div>
                                                <div class="font-medium text-gray-800 dark:text-white/90"
                                                    x-text="log.technician?.full_name || 'Unknown Technician'"></div>
        ...
        @push('scripts')
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    if (workLogForm) {
                        workLogForm.addEventListener('submit', async function(e) {
                            ...
                            try {
                                ...
                                if (result.success) {
                                    // Close modal
                                    Alpine.store('workLogModalOpen', false);
                                    showWorkLogModal = false;

                                    // Show success message
                                    showToast('success', result.message);

                                    // Reload work logs section
                                    loadWorkLogs();

                                    // Update status badge if it exists
                                    updateStatusBadge(result.new_status, result.new_status_text);

                                    // Reset form
                                    this.reset();
                                    document.getElementById('log_date').value = new Date().toISOString().split(
                                        'T')[0];
                                    ...
                                    showToast('error', result.message || 'Failed to save work log.');
                                }
                            } catch (error) {
                                console.error('Error:', error);
                    ...
                    function updateStatusBadge(newStatus, newStatusText) {
                        const statusBadge = document.querySelector('[class*="bg-"]:contains("Status")');
                        if (statusBadge) {
                    ...
                    // Function to load work logs via AJAX
                    function loadWorkLogs() {
                        fetch(`{{ route('work-logs.by-request', $maintenanceRequest) }}`)
                            .then(response => response.json())
                            .then(data => {
                                if (data.success) {
                                    updateWorkLogsDisplay(data.workLogs, data.total_time);
                                }
                            })
                            .catch(error => {
                    ...
                    // Function to update work logs display
                    function updateWorkLogsDisplay(workLogs, totalTime) {
                        const container = document.querySelector('[x-data="{ workLogs: [] }"]');
                        if (container) {
                            container.__x.$data.workLogs = workLogs;
                        }

                        // Update total time display
                        const totalTimeElement = document.getElementById('total-work-time');
                        if (totalTimeElement) {
        ...
        `;
                    ...
                    window.deleteWorkLog = async function(workLogId) {
                        ...
                        try {
                            ...
                            if (result.success) {
                                showToast('success', 'Work log deleted successfully!');
                                loadWorkLogs();
                                // Reload page to update status if needed
                                window.location.reload();
                            } else {
                                showToast('error', result.message || 'Failed to delete work log.');
                            }
                        } catch (error) {
                            console.error('Error:', error);
                            showToast('error', 'An error occurred. Please try again.');
