...
@section('content')
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <form method="POST" action="{{ route('maintenance-requests.update', $maintenanceRequest) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                                ...
                                <div>
                                    ...
                                    <select name="item_id"
                                        ...
                                        @foreach($items as $item)
                                            <option value="{{ $item->id }}" {{ old('item_id', $maintenanceRequest->item_id) == $item->id ? 'selected' : '' }}>
                                ...
                                <div>
                                    ...
                                    <select name="issue_type_id"
                                        ...
                                        @foreach($issueTypes as $issueType)
                                            <option value="{{ $issueType->id }}" {{ old('issue_type_id', $maintenanceRequest->issue_type_id) == $issueType->id ? 'selected' : '' }}>
                                                {{ $issueType->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div>
                            ...
                            <select name="status"
                                ...
                                @foreach(App\Models\MaintenanceRequest::getStatusOptions() as $key => $value)
                                    <option value="{{ $key }}" {{ old('status', $maintenanceRequest->status) == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="assigned_to"
                                ...
                                @foreach($technicians as $tech)
                                    <option value="{{ $tech->id }}" {{ old('assigned_to', $maintenanceRequest->assigned_to) == $tech->id ? 'selected' : '' }}>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        fileInput.addEventListener('change', function(event) {
            ...
            // Clear previous list
            fileList.innerHTML = '<h5 class="text-sm font-medium text-gray-700 dark:text-gray-300">New Files (<span id="file-count">0</span>)</h5>';
