 @php
     ...
     $hasPendingApprovalRequest = $maintenanceRequest->approvalRequests()->where('status', 'forward')->exists();
     // Determine if approval buttons should be shown
     $showApprovalButtons = false;

     if ($needsApproval || $hasPendingApprovalRequest) {
         // Only show approval buttons if status is pending or waiting_approval
         // this for division chairman
         if (
             in_array($maintenanceRequest->status, ['pending', 'waiting_approval']) &&
             $user->isDivisionChairman() &&
             $maintenanceRequest->user->division_id === $user->division_id
         ) {
         ...
         //here for cluster chairman
         if (
             in_array($maintenanceRequest->status, ['pending', 'waiting_approval']) &&
             $user->isClusterChairman() &&
             optional($maintenanceRequest->user->division)->cluster_id === $user->cluster_id
         ) {
             $approver = $maintenanceRequest->getNextApprover();
             $canApprove =
                 ($approver && auth()->id() == $approver->id) || auth()->user()->can('maintenance_requests.approve');
